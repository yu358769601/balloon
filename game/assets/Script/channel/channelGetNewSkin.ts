...
export default class ChannelGetNewSkin extends ChannelBase implements IChannelSkinTrialDialog{
    ...
    按钮缩放() {
        if (ControlCommercial.getSceneData(
            ...
            UtilsAction.btnAn(this.bindComponent.胜利_看广告领取)
        }
    ...
    激励广告点击区域参数控制() {
        ...
        if (
            ...
            this.bindComponent.胜利_看广告领取实际点击.height*=(ControlNum/100)
        }
    }
    激励广告点击区域时间间隔控制(is) {
    ...
    @property(
        {
            type: GetNewSkin,
            displayName: "GetNewSkin_放这里"
        }
    )    // call cc.Enum
    bindComponent : GetNewSkin = null
