...
export default class SuperGetDialog extends BaseDialog implements IChannelBase{
    ...
    onAddCountData(selfName,data){
        ...
        UtilsNode.show( this.引导_小手指指点,false)
        if (this.addCountData.length>=JsonManager.passSettingjson.superGetCountMax) {
            ...
            Emitter.fire("onShowItemSuperItemAD")
        }
    ...
    async setData(data) {
        ...
        UtilsNode.show( this.引导_小手指指点,true)
        UtilsAction.hand( this.引导_小手指指点)
        // this.initLoad()
    ...
    async initView() {
        ...
        this.引导_小手指指点 = GetNode.getNode(data)
