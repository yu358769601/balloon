...
export default class SkinTrialDialog extends BaseDialog  implements IChannelBase{
   ...
   async setData(data) {
       ...
       ccLog.log("进来的数据是", this.data.data)
       if (this.data) {

           this.结算_送橡皮底板展示3.spriteFrame = await LoadManage.getSpriteForName("shopBig_"+this.data.data.rubber.item.name)
       }
  ...
  async  initLoad(){
    ...
    lookDialogsuccessfulCallback(data){
        ...
        let name = data.data.self.data.data.rubber.item.name
   ...
   async initView() {
       ...
       this.皮肤试用_关闭 = GetNode.getNode(data)
       data = {
           type: GetNodeType.纯查找,
